...
public class Show_Tools extends JFrame
{
    JButton button1;
    public Show_Tools()
    {
        List<Tools> allProducts_List = new ArrayList<>();
        String cnnString =
        ...
        try
        {
            ...
            ResultSet myRs = myStmt.executeQuery("SELECT * FROM FullTools");
            while (myRs.next()) {
                allProducts_List.add(new Tools(myRs.getString("idProduct"), myRs.getString("productName"), myRs.getInt("quantity"), myRs.getDouble("price"),myRs.getString("toolType") ));
            }
            myRs.close();
            myStmt.close();
            myconn.close();
        ...
        for(int i = 0; i < allProducts_List.size(); i++)
        {
            ...
            data[i][4] = allProducts_List.get(i).getToolType();
        }
