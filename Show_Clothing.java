...
public class Show_Clothing extends JFrame
{
    JButton button1;
    public Show_Clothing()
    {
        List<Clothing> allProducts_List = new ArrayList<>();
        String cnnString =
        ...
        try
        {
            ...
            ResultSet myRs = myStmt.executeQuery("SELECT * FROM FullClothing");
            while (myRs.next()) {
                allProducts_List.add(new Clothing(myRs.getString("idProduct"), myRs.getString("productName"), myRs.getInt("quantity"), myRs.getDouble("price"), myRs.getString("material"), myRs.getString("clothingType"), myRs.getString("size"), myRs.getString("sex")));
            }
            myRs.close();
            myStmt.close();
            myconn.close();
        ...
        for(int i = 0; i < allProducts_List.size(); i++)
        {
            ...
            data[i][4] = allProducts_List.get(i).getMaterial();
            data[i][5] = allProducts_List.get(i).get_type();
            data[i][6] = allProducts_List.get(i).get_size();
            data[i][7] = allProducts_List.get(i).get_sex();
        }
