...
public class Show_Food extends JFrame
{
    JButton button1;
    public Show_Food()
    {
        List<Food> allProducts_List = new ArrayList<>();
        String cnnString =
        ...
        try
        {
            ...
            ResultSet myRs = myStmt.executeQuery("SELECT * FROM FullFood");
            while (myRs.next()) {
                allProducts_List.add(new Food(myRs.getString("idProduct"), myRs.getString("productName"), myRs.getInt("quantity"), myRs.getDouble("price"),myRs.getString("expirationDate") ));
            }
            myRs.close();
            myStmt.close();
            myconn.close();
        ...
        for(int i = 0; i < allProducts_List.size(); i++)
        {
            ...
            data[i][4] = allProducts_List.get(i).getExpirationDate();
        }
