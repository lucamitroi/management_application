...
public class Show_Electronic_Devices extends JFrame
{
    JButton button1;
    public Show_Electronic_Devices()
    {
        List<Electronic_Devices> allProducts_List = new ArrayList<>();
        String cnnString =
        ...
        try
        {
            ...
            ResultSet myRs = myStmt.executeQuery("SELECT * FROM FullElectronicDevice");
            while (myRs.next()) {
                allProducts_List.add(new Electronic_Devices(myRs.getString("idProduct"), myRs.getString("productName"), myRs.getInt("quantity"), myRs.getDouble("price"), myRs.getString("electronicDeviceType"), myRs.getString("brand")));
            }
            myRs.close();
            myStmt.close();
            myconn.close();
        ...
        for(int i = 0; i < allProducts_List.size(); i++)
        {
            ...
            data[i][4] = allProducts_List.get(i).getType();
            data[i][5] = allProducts_List.get(i).getBrand();
